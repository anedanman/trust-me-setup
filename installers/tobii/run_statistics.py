...
def from_timer_to_date(timer_string,y,month,d, h,m,s,micro):
    recS=datetime(year=y,month=month,day=d,hour=h, minute=m, second=s,microsecond=micro)
...
def from_timer_to_timestamp(timer_string,y,month,d, h,m,s,micro):
    recS=datetime(year=y,month=month,day=d,hour=h, minute=m, second=s,microsecond=micro)
    # the timer is expressed in milliseconds so we need to retrieve the microseconds
    list_info=str(recS+ timedelta(microseconds=float(timer_string)*1000)).split()[1].split(':')
    return "{}:{}:{}".format(list_info[0],list_info[1], list_info[2].split('.')[0])

def get_millis_elapsed(timer_string,y,month,d, h,m,s,micro):
    recS=datetime(year=y,month=month,day=d,hour=h, minute=m, second=s,microsecond=micro)
    # the timer is expressed in milliseconds so we need to retrieve the microseconds
    return str(recS+ timedelta(microseconds=float(timer_string)*1000)).split()[1].split(':')[-1].split('.')[1]
...
def create_csv(path='./tobii/recordings/'):
    ...
    files = [f for f in os.listdir(path) if f.endswith('.tsv') and f.startswith('calibration')]
    # print(files)
    df=None
    list_of_data=[]
    columns=['Timestamp',
    ...
    for index,file in enumerate(files):
        ...
        df['Year_Month_day']=df['Timestamp'].apply(from_timer_to_date, args=(y,month,d,h,m,s,micro))
        df['MicrosecElapsed']= df['Timestamp'].apply(get_millis_elapsed, args=(y,month,d,h,m,s,micro))
        df['Timestamp']= df['Timestamp'].apply(from_timer_to_timestamp, args=(y,month,d,h,m,s,micro))
        # print(df)
    ...
    merged_df['PupilSizeLeft'] = merged_df['PupilSizeLeft'].astype(float, errors='ignore')
    merged_df['PupilSizeRight'] = merged_df['PupilSizeRight'].astype(float, errors='ignore')
    merged_df['ValidityLeft'] = merged_df['ValidityLeft'].astype(bool, errors='ignore')
    merged_df['ValidityRight'] = merged_df['ValidityRight'].astype(bool, errors='ignore')
