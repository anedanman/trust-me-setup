...
VERBOSE = False # Debug
SLEEP_TIME = 10 # set glocal sleep time in seconds

# define subprocess
import subprocess
command_very_happy = "/bin/bash -c 'echo \"$(date) Very happy.\" >> ./streamdeck/Recording/$(date +\"%Y-%m-%d\")'"
command_happy = "/bin/bash -c 'echo \"$(date) Happy.\" >> ./streamdeck/Recording/$(date +\"%Y-%m-%d\")'"
command_fine = "/bin/bash -c 'echo \"$(date) Fine.\" >> ./streamdeck/Recording/$(date +\"%Y-%m-%d\")'"
command_unhappy = "/bin/bash -c 'echo \"$(date) Unhappy.\" >> ./streamdeck/Recording/$(date +\"%Y-%m-%d\")'"
command_angry = "/bin/bash -c 'echo \"$(date) Angry.\" >> ./streamdeck/Recording/$(date +\"%Y-%m-%d\")'"
...
def timer_function(deck):
    global NEED_PRESS
    while deck.is_open():
        for i in range(SLEEP_TIME): # Wait for SLEEP_TIME seconds
            if not deck.is_open():
                break
            time.sleep(1)
        NEED_PRESS = True
